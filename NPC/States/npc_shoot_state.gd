...
func update(_delta: float):
	if not target_manager.target:
		return
		
	if target_manager.target.global_position.distance_to(npc.global_position) < 20.0 and shoot_cooldown_timer.is_stopped() and !playing_hit_animation:
		shoot()
		shoot_cooldown_timer.start()
